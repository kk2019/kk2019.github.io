...
gbx.define("gbx/net/connector", ["gbx/hubs", "gbx/net/messagecommand", "gbx/net/messagecommand_des"], function(hubs, CMD, CMD_DES) {
	...
	var netConnector = {
		...
		connect: function() {
			if (variables.host == "" || variables.port == 0) throw "must set rpc server config before connect.";
			...
			function onDisconnected(reason) {
				hubs.publish("global.net_connect.disconnected", reason);
			}
			console.log("连接网络:"+variables.host+":"+variables.port);
			return gtea.net.NetMsgMgr.connect(variables.host, variables.port, onConnected, onDisconnected);
		},
