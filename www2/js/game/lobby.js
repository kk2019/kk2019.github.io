gbx.define("game/lobby/lobbypop/account_safe", ["gbx/hubs", "gbx/render/display/sprite", "comm/ui/sound_button", "gbx/render/display/text", "comm/base/command_ui", "comm/helper/playerdata_mgr", "comm/helper/account_mgr", "comm/ui/ui_utils", "game/lobby/lobbypop/bind_phone", "game/lobby/lobbypop/change_name", "game/lobby/lobbypop/change_head", "game/lobby/lobbypop/change_pass", "game/lobby/lobbypop/bind_parent", "conf/ui_locallization", "gbx/net/connector", "gbx/net/messagecommand", "comm/helper/scene_mgr"], function (hubs, Sprite, Button, Text, BaseUI, playerDataMgr, accountMgr, uiUtils, BindPhone, ChangeName, ChangeHead, ChangePass, BindParent, uiLocallization, server, CMD, sceneMgr) {
	var AccountSafe = BaseUI.extend({
		init: function () {
			...
			for (var i = 0; i < 4; i++) {
				var shadow;
				if (i == 3) {
					shadow = uiUtils.createButton("assets/main/public/item_bg.png", 1, i.toString());
					shadow.setClickHandler(this, this.OnClickExitLobby);
					shadow.label = "退出游戏";
					shadow.labelSize = 30;
					shadow.width = 380;
					shadow.height = 86;
					shadow.labelColors = "#ffffff";
				} else {
					shadow = uiUtils.createSprite("assets/main/public/item_bg.png", "");
...
gbx.define("game/lobby/lobbypop/bank_draw_encharge", ["gbx/net/connector", "gbx/net/messagecommand", "gbx/net/errorcode", "gbx/render/display/sprite", "comm/ui/sound_button", "gbx/render/display/text", "gbx/render/ui/panel", "comm/base/command_ui", "game/lobby/lobbypop/bank_pass", "comm/helper/playerdata_mgr", "comm/ui/ui_utils", "comm/ui/top_message", "conf/locallization", "conf/ui_locallization"], function (server, CMD, ERROR_CODE, Sprite, Button, Text, Panel, BaseUI, BankPass, playerDataMgr, uiUtils, topMessage, LocalLization, uiLocallization) {
	var BankDraw = BaseUI.extend({
		...
		refresh: function () {
			this._number1 = playerDataMgr.getMyCarryGold();
			this._number2 = (playerDataMgr.getMyGold() * 100 - playerDataMgr.getMyCarryGold() * 100) / 100;
			this.getChildByName("usable_limit").text = this._number1;
			this.getChildByName("deposit").text = this._number2;
		}
	});
	return BankDraw;
});
...
gbx.define("game/lobby/lobbypop/bank_setting", ["gbx/net/connector", "gbx/net/messagecommand", "gbx/net/errorcode", "gbx/net/errorcode_des", "gbx/net/net_tool", "gbx/render/display/sprite", "comm/ui/sound_button", "gbx/render/display/text", "gbx/render/ui/panel", "comm/base/command_ui", "comm/ui/ui_utils", "game/lobby/lobbypop/income_expend_detail", "game/lobby/lobbypop/bank_draw_encharge", "game/lobby/lobbypop/bank_charge", "game/lobby/lobbypop/draw_cash", "game/lobby/lobbypop/bind_bank_card", "game/lobby/lobbypop/unlock_bank_card", "comm/helper/playerdata_mgr", "conf/ui_locallization", "gbx/bridge", "comm/helper/bank_mgr"], function (server, CMD, ERROR_CODE, ERROR_CODE_DES, netTool, Sprite, Button, Text, Panel, BaseUI, uiUtils, InComExpDetail, DrawEncharge, BankCharge, DrawCash, BindCard, UnLockBank, playerDataMgr, uiLocallization, bridge, bankMgr) {
	var BankSetting = BaseUI.extend({
		init: function () {
			...
			if (this.hasCard) {
				if (bankData == null) {
					...
					cardName.text = bankData.belong + "-" + bankData.cardName
				}
			}
		...
		eventUpdateCardInfo: function () {
			...
			if (data == null) {
				iconName = "alipay.png";
				child2.getChildByName("cardName").text = "支付宝";
			} else {
				iconName = "quick.png";
		...
		eventUpdateMyDeposit: function () {
			var deposit = parseInt(playerDataMgr.getMyGold() * 100 - playerDataMgr.getMyCarryGold() * 100) / 100;
			this.getChildByName("myMoney").text = deposit;
			this.getChildByName("usable_draw_limit").text = parseInt((playerDataMgr.getMyGold() * 100 - deposit * 100) / 10000) * 100;
			this.getChildByName("usable_limit").text = (playerDataMgr.getMyGold() * 100 - deposit * 100) / 100;
		},
		eventOpenDetail: function () {
			var param = {
				gamever: 12001,
				appid: 12001,
				token: playerDataMgr.origData.TokenId,
				uid: playerDataMgr._myData.uid,
				pfid: 5,
				time: new Date().getTime()
			};
			netTool.createHttpRequest("User/GetBillList", param, this, function (data) {
				if (data.response) {
					data = unescape(data.response.replace(/\\u/g, '%u'));
					data = JSON.parse(data);
				}
				this.getChildByName("incomExpDetailPanel").visible = true;
				this.getChildByName("incomExpDetailPanel").setData(data.data);
			});
		},
...
gbx.define("game/lobby/lobbypop/bind_bank_card", ["gbx/hubs", "gbx/net/connector", "gbx/net/messagecommand", "gbx/net/errorcode", "gbx/render/display/sprite", "comm/ui/sound_button", "gbx/render/display/text", "comm/base/command_ui", "comm/helper/playerdata_mgr", "comm/helper/account_mgr", "comm/ui/ui_utils", "conf/ui_locallization", "comm/helper/bank_mgr", "gbx/net/net_tool"], function (hubs, server, CMD, ERROR_CODE, Sprite, Button, Text, BaseUI, playerDataMgr, accountMgr, uiUtils, uiLocallization, bankMgr, netTool) {
	var BindBankCard = BaseUI.extend({
		init: function () {
			...
			surebtn.setClickHandler(this, this.OnClick);
			this.addChild(surebtn);
			this.checkbindCardNumber();
		},
		checkbindCardNumber: function (number, playerName) {
			var param = {
				gamever: 12001,
				appid: 12001,
				token: playerDataMgr.origData.TokenId,
			...
			netTool.createHttpRequest("User/GetCard", param, this, function (data) {
				if (data.response) {
					data = unescape(data.response.replace(/\\u/g, '%u'));
					data = JSON.parse(data);
				}
				if (data.data.length > 0) {
					playerDataMgr.setBankCardNumber(data.data[0].cardid);
		...
		bindCardNumber: function (number, playerName) {
			...
			var param = {
				...
				cardid: cardNumber,
				cardname: bankName,
				cardphone: playerDataMgr.origData.MobilePhone,
				time: new Date().getTime()
			};
			netTool.createHttpRequest("User/BindCard", param, this, function (data) {
				if (data.response) {
					data.response = unescape(data.response.replace(/\\u/g, '%u'));
				}
		...
		bindCard: function (cardNumber, playerName) {
			...
			if (ret) {
				...
				//有可能是支付宝号
				this.bindCardNumber(cardNumber, playerName);
			}
		}
	});
	return BindBankCard;
...
gbx.define("game/lobby/lobbypop/draw_cash", ["gbx/hubs", "gbx/net/connector", "gbx/net/messagecommand", "gbx/net/errorcode", "gbx/render/display/sprite", "comm/ui/sound_button", "gbx/render/display/text", "comm/base/command_ui", "comm/helper/playerdata_mgr", "comm/helper/account_mgr", "comm/ui/ui_utils", "conf/ui_locallization", "comm/helper/bank_mgr"], function (hubs, server, CMD, ERROR_CODE, Sprite, Button, Text, BaseUI, playerDataMgr, accountMgr, uiUtils, uiLocallization, bankMgr) {
	var DrawCash = BaseUI.extend({
		...
		drawCash: function (number) {
			var sendMsg = gtea.protobuf.encode("RequestTakeCash", {
				Money: number * 100
			}, 1, 8);
			gtea.net.ProtobufConverter.getInstance().setCallback(CMD.GetCash, 1, 8, "ReplyTakeCash");
			server.requestToGame(CMD.GetCash, sendMsg, this, function (errorcode, data) {
				if (data.ErrorCode == ERROR_CODE.OK) {
					data.itemProto = {};
					data.itemProto.DType = 4;
					data.itemProto.id = 1;
					data.itemProto.count = data.Money / 100;
					playerDataMgr.dealItemProto(data.itemProto);
		...
		refreshCardInfo: function () {
			var data = bankMgr.getCardData(this._cardAccount);
			if (data == null) {
				this.getChildByName("cardName").text = "支付宝";
			} else {
...
gbx.define("game/lobby/lobbypop/gambling_details", ["gbx/render/display/sprite", "comm/ui/sound_button", "gbx/render/display/text", "comm/ui/ui_utils", "gbx/render/ui/panel", "conf/ui_locallization"], function (Sprite, Button, Text, uiUtils, Panel, uiLocallization) {
	var GamblingDetail = Sprite.extend({
		...
		_putOne: function (data, index) {
			...
			newDate.setTime(rdata.Time * 1000);
			...
			var before = uiUtils.createSimpleText(uiUtils.getFloatByPower(rdata.TotalMoney, 1), "before");
			...
			var i = 0;
			var result = "";
			result += uiLocallization.banker + ": " + rdata.BankPoint + uiLocallization.point + " \n" + uiLocallization.farmer + ": " + rdata.PlayPoint + uiLocallization.point + " \n";

			var bankCard = [rdata.BankCard[0] >> 8, rdata.BankCard[1] >> 8];
			var farmCard = [rdata.PlayCard[0] >> 8, rdata.PlayCard[1] >> 8];

			rdata.bankerPair = bankCard[0] == bankCard[1];
			rdata.farmerPair = farmCard[0] == farmCard[1];
			rdata.winType = 3;
			if (rdata.BankPoint > rdata.PlayPoint) {
				rdata.winType = 1;
			} else if (rdata.BankPoint < rdata.PlayPoint) {
				rdata.winType = 1;
			}
			...
			var mybet = uiUtils.createSimpleText(rdata.GameChips, "mybet");
			...
			var moneyDelta = uiUtils.createSimpleText(rdata.TotalMoney - rdata.GameChips, "moneyDelta");
			moneyDelta.pos(295, 37 + posY);
			moneyDelta.align = "center";
			moneyDelta.fontSize = 18;
			moneyDelta.color = "#E9E0C3";
			root.addChild(moneyDelta);
			var after = uiUtils.createSimpleText(rdata.TotalMoney, "after");
...
gbx.define("game/lobby/lobbypop/income_expend_detail", ["gbx/render/display/sprite", "comm/ui/sound_button", "gbx/render/display/text", "gbx/render/ui/panel", "comm/base/command_ui", "comm/ui/ui_utils", "conf/ui_locallization"], function (Sprite, Button, Text, Panel, BaseUI, uiUtils, uiLocallization) {
	var IncomeExpend = BaseUI.extend({
		...
		_putOne: function (data, index) {
			...
			//newDate.setTime(rdata.date);
			//var time = newDate.getFullYear() + "." + (newDate.getMonth() + 1) + "." + newDate.getDate() + "\n" + newDate.getHours() + ":" + newDate.getMinutes() + ":" + newDate.getSeconds();
			var timelab = uiUtils.createSimpleText(rdata.ctime, "time");
...
gbx.define("game/lobby/lobbypop/system_setting", ["gbx/render/display/sprite", "comm/ui/sound_button", "comm/helper/playerdata_mgr", "gbx/net/messagecommand", "gbx/net/connector", "gbx/net/net_tool", "gbx/net/errorcode", "comm/ui/ui_utils", "comm/ui/top_message", "gbx/net/errorcode_des", "platform/platform_manager"], function (Sprite, Button, playerDataMgr, CMD, server, netTool, ERROR_CODE, uiUtils, topMessage, ERROR_CODE_DES, platformManager) {
	var SystemSetting = Sprite.extend({
		...
		_openDetail: function () {
			netTool.createHttpRequest("User/GetResultRecord", { uid: playerDataMgr.getMyUID(), token: playerDataMgr.origData.TokenId, page: 1, pagenum: 100 }, this, function (retData) {
				...
				this.publish("lobby.setting.play.open_gambling", JSON.parse(retData.response));
			});
...
gbx.define("game/lobby/scene/login", ["gbx/net/connector", "gbx/net/messagecommand", "gbx/net/errorcode", "gbx/net/errorcode_des", "gbx/net/net_tool", "game/lobby/ui/login", "gbx/render/display/sprite", "comm/ui/connect_loading", "comm/helper/sound_mgr", "comm/helper/scene_mgr", "comm/helper/playerdata_mgr", "comm/helper/account_mgr", "comm/helper/room_mgr", "comm/ui/top_message", "gbx/hubs", "gbx/bridge", "conf/locallization", "conf/version", "gbx/util", "comm/net/msg_receiver", "comm/helper/conf_mgr", "platform/platform_manager", "conf/platform"], function (server, CMD, ERROR_CODE, ERROR_CODE_DES, netTool, LoginUI, Sprite, loadingUI, soundMgr, sceneMgr, playerDataMgr, accountMgr, roomMgr, topMessage, hubs, bridge, Localization, version, util, systemReceiver, confMgr, platformManager, Platform) {
	...
	var stepGetUserData = function () {
		...
		if (myGlobalData.isSign) {
			...
			server.requestToGame(CMD.GetUserInfoCommand * 9, sendData, this, function (errorcode, mData) {
				if (mData.ErrorCode == ERROR_CODE.OK) {
					...
					if (mData.ErrorCode == 1) {
						...
						showTips("重复登录");
					} else if (mData.ErrorCode == 7) {
						showTips("服务器关闭");
					} else if (mData.ErrorCode == 8) {
						showTips("手机号未绑定");
					} else if (mData.ErrorCode == 11) {
						showTips("验证错误");
					} else {
						showTips("登录失败");
					}
			...
			server.requestToGame(CMD.GetUserInfoCommand, sendData, this, function (errorcode, retData) {
				if (retData.ErrorCode == ERROR_CODE.OK) {
					var msgData = retData;

					var request = new XMLHttpRequest;
					var requestURL = "http://bbdown.oss-cn-hangzhou.aliyuncs.com/ipConfig/agent/" + msgData.PlayerProp;
					...
					request.onreadystatechange = function () {
						if (request.readyState == 4) {
							if (request.status == 200) {
								...
								if (result.result == "ok") {
									localStorage["serverIP"] = result.ipNumber;
									localStorage["serverPort"] = result.h5post;
								}
							} else {
					...
					if (retData.ErrorCode == 1) {
						...
						showTips("重复登录");
					} else if (retData.ErrorCode == 7) {
						showTips("服务器关闭");
					} else if (retData.ErrorCode == 8) {
						showTips("手机号未绑定");
					} else {
						showTips("登录失败");
					}
	...
	return {
		...
		show: function (isInit, reconnect) {
			...
			loginUI.addToStage();
			if (bridge.getPlatform() === "web") loginUI.toggleForm("register");
			else loginUI.toggleForm("login");
	...
	var LobbyUI = BaseUI.extend({
		init: function () {
			...
			if (playerDataMgr.getMyData().serverID > 0) {
				...
				var sendData = gtea.protobuf.encode("RequestInsureLogin", {
					Password: myGlobalData.md5PWD
				}, 1, 10);
				gtea.net.ProtobufConverter.getInstance().setCallback(CMD.GetUserInfoCommand * 1000, 1, 10, "ReplyInsureLogin");
				server.requestToGame(CMD.GetUserInfoCommand * 1000, sendData, this, function (errorcode, mData) {
					playerDataMgr.setMyCarryGold(parseInt(mData.Score) / 100);
					playerDataMgr.setMyGold((parseInt(mData.InsureScore) + parseInt(mData.Score)) / 100);
					this.publish("lobby.game.updateMyInfo");
					this.publish("game_baccarat.setting.updateMyDeposit");
				});
			}
		...
		eventOpenBankcharge: function () {
			var account = accountMgr.getUserData().account;
			platformManager.tryRechargeOrder_qrCode(account, playerDataMgr.getMyUID(), "12001", 9);
		},
...
gbx.define("game/lobby/ui/login", ["conf/system", "comm/base/command_ui", "gbx/render/display/sprite", "gbx/render/display/animation", "gbx/render/display/text", "gbx/render/ui/button", "gbx/render/ui/textinput", "gbx/render/ui/checkbox", "gbx/bridge", "comm/ui/top_message", "conf/version", "conf/ui_locallization", "comm/helper/playerdata_mgr", "comm/helper/account_mgr", "game/lobby/parts/find_password", "game/lobby/parts/country_code", "conf/platform", "gbx/hubs", "comm/ui/ui_utils", "gbx/net/net_tool", "gbx/net/errorcode_des", "game/lobby/parts/password"], function (sys, BaseUI, Sprite, Animation, Text, Button, TextInput, Checkbox, bridge, topMessage, version, uiLocallization, playerDataMgr, accountMgr, FindPasswordForm, countryCode, Platform, hubs, uiUtils, netTool, ERROR_CODE_DES, PasswordForm) {
	var LoginUI = BaseUI.extend({
		init: function (account, pass, country, checkAccount, checkPassword) {
			...
			var countryForm = new Sprite;
			countryForm.pos(-63, 115);
			countryForm.name = "country_form";
			this.addChild(countryForm);
			var wordCountry = new Text;
			wordCountry.text = "国家/地区";
			wordCountry.pos(sx - 100, sy + 25);
			wordCountry.fontSize = 27;
			wordCountry.color = "#ffffff";
			wordCountry.pos(sx - 100, sy + 25);
			countryForm.addChild(wordCountry);
			var countryName = this._getCountryByCode(country);
			var countryLabel = uiUtils.createSimpleText(countryName, "country");
			countryLabel.pos(sx + 50, sy + 25);
			countryLabel.color = "#ffffff";
			countryLabel.fontSize = 27;
			countryForm.addChild(countryLabel);
			var arrow = new Button;
			arrow.setSkinImage("assets/main/login/arrow.png");
			arrow.stateNum = 1;
			arrow.name = "choose_country_btn";
			arrow.setClickHandler(this, this.onClickCountry);
			arrow.pos(sx + 358, sy + 27);
			countryForm.addChild(arrow);
			var countryPanel = new Sprite;
			countryPanel.name = "countryPanel";
			countryPanel.pos(175, sy + 180);
			countryPanel.visible = false;
			countryPanel.zOrder = 1;
			countryPanel.graphics.drawRect(0, 0, 256, 220, "#25221C", 1);
			this.addChild(countryPanel);
			var maskBtn_1 = uiUtils.createMask(this, true);
			maskBtn_1.pos(-1500, -1500);
			maskBtn_1.name = "maskBtn_1";
			maskBtn_1.setClickHandler(this, this.onClickCountry);
			countryPanel.addChild(maskBtn_1);
			var listCountry = uiUtils.createNormalList(1, 10, 0, "v", 250, 220, 250, 45);
			listCountry.array = countryCode.getCountryCode();
			listCountry.name = "listCountry";
			listCountry.setRenderHandler(this, this._renderCountryCode);
			countryPanel.addChild(listCountry);
			var loginForm = new Sprite;
			loginForm.pos(-63, 115);
			loginForm.visible = false;
			loginForm.name = "login_form";
			var accountBG = uiUtils.createSprite("assets/main/login/denglu_dk1.png");
			accountBG.pos(sx - 100, sy + 80);
			loginForm.addChild(accountBG);
			var pwdeBG = uiUtils.createSprite("assets/main/login/denglu_dk2.png");
			pwdeBG.pos(sx - 100, sy + 190);
			loginForm.addChild(pwdeBG);
			this.addChild(loginForm);
			var wordAccount = new Text;
			wordAccount.text = "账号";
			wordAccount.fontSize = 27;
			wordAccount.color = "#ffffff";
			wordAccount.pos(sx - 75, sy + 110);
			loginForm.addChild(wordAccount);
			var wordPass = new Text;
			wordPass.text = "密码";
			wordPass.name = "wordPass";
			wordPass.fontSize = 27;
			wordPass.color = "#ffffff";
			wordPass.pos(sx - 75, sy + 210);
			loginForm.addChild(wordPass);
			var inputUsername = new TextInput;
			inputUsername.prompt = uiLocallization.login_account;
			inputUsername.size(265, 70);
			inputUsername.fontSize = 27;
			inputUsername.color = "#ffffff";
			inputUsername.pos(sx + 25, sy + 90);
			inputUsername.name = "txt_username";
			loginForm.addChild(inputUsername);
			if (checkAccount && pass !== "") {
				if (account.indexOf(country) != -1 && account.indexOf(country) == 0) {
					account = account.substring(country.length);
				}
				inputUsername.text = account;
			}
			var inputPassword = new TextInput;
			inputPassword.prompt = uiLocallization.login_password;
			inputPassword.type = "password";
			inputPassword.size(265, 70);
			inputPassword.fontSize = 27;
			inputPassword.color = "#ffffff";
			inputPassword.pos(sx + 25, sy + 190);
			inputPassword.name = "txt_password";
			loginForm.addChild(inputPassword);
			if (checkPassword) inputPassword.text = pass;
			var guestLoginBtn = new Button;
			guestLoginBtn.setSkinImage("assets/main/login/login_button_guest.png");
			guestLoginBtn.stateNum = 2;
			guestLoginBtn.pos(sx + 225, sy + 96);
			guestLoginBtn.name = "guestLoginBtn";
			guestLoginBtn.scale(1.35);
			guestLoginBtn.visible = false;
			guestLoginBtn.setClickHandler(this, this.onClickGuestLogin);
			loginForm.addChild(guestLoginBtn);
			var userLoginBtn = new Button;
			userLoginBtn.label = "登录";
			...
			userLoginBtn.pos(sx + 330, 740);
			userLoginBtn.setClickHandler(this, this.onClickUserLogin);
			loginForm.addChild(userLoginBtn);
			var turnToSignupBtn = new Button;
			turnToSignupBtn.label = "注册";
			turnToSignupBtn.labelColors = "#ffffff,#ffffff,#ffffff";
			turnToSignupBtn.labelSize = 40;
			turnToSignupBtn.labelPadding = "-20,0,0,0";
			turnToSignupBtn.setSkinImage("assets/main/login/sss3-fs8.png");
			turnToSignupBtn.stateNum = 1;
			turnToSignupBtn.pivotToCenter();
			turnToSignupBtn.pos(sx + 36, 740);
			turnToSignupBtn.setClickHandler(this, this.onClickTurnToSignup);
			loginForm.addChild(turnToSignupBtn);
			var checkY = 630;
			var saveUsername = new Checkbox;
			saveUsername.setSkinImage("assets/main/login/login_account.png");
			saveUsername.stateNum = 2;
			saveUsername.pos(sx - 100, checkY);
			saveUsername.name = "cb_username";
			loginForm.addChild(saveUsername);
			if (checkAccount) saveUsername.selected = true;
			var savePassword = new Checkbox;
			savePassword.setSkinImage("assets/main/login/login_password.png");
			savePassword.stateNum = 2;
			savePassword.pos(sx + 70, checkY);
			savePassword.name = "cb_password";
			loginForm.addChild(savePassword);
			if (checkPassword) savePassword.selected = true;
			var findPass = new Button;
			findPass.pivotToCenter();
			findPass.size(200, 70);
			var word_reg = uiUtils.createSprite("assets/main/login/forget_pass.png");
			word_reg.pos(0, 5);
			findPass.addChild(word_reg);
			findPass.pos(sx + 240, checkY);
			findPass.setClickHandler(this, this.onClickFindPass);
			loginForm.addChild(findPass);
			var registerForm = new Sprite;
			registerForm.pos(-63, 115);
			registerForm.visible = false;
			registerForm.name = "register_form";
			this.addChild(registerForm);
			var phoneBG = uiUtils.createSprite("assets/main/login/denglu_dk1.png");
			phoneBG.pos(sx - 100, sy + 80);
			registerForm.addChild(phoneBG);
			var verifyBG = uiUtils.createSprite("assets/main/login/denglu_dk2.png");
			verifyBG.pos(sx - 100, sy + 190);
			registerForm.addChild(verifyBG);
			var plus = new Text;
			plus.name = "plus";
			plus.text = "+86";
			plus.color = "#ffffff";
			plus.fontSize = 27;
			plus.pos(sx - 95, sy + 110);
			registerForm.addChild(plus);
			//for (var i = 0; i < 4; i++) {
			//	var num = uiUtils.createSprite("assets/main/login/num/Number_0.png", "");
			//	num.pos(sx - 100 + (i + 1) * 20, sy + 110);
			//	num.name = "num" + i;
			//	num.visible = false;
			//	registerForm.addChild(num)
			//}

			this.setSpriteNum(country);
			var wordVerifyCode = new Text;
			wordVerifyCode.text = "验证码";
			wordVerifyCode.color = "#ffffff";
			wordVerifyCode.fontSize = 27;
			wordVerifyCode.pos(sx - 90, sy + 220);
			registerForm.addChild(wordVerifyCode);
			var inputAccount = new TextInput;
			inputAccount.prompt = uiLocallization.login_phone;
			inputAccount.size(265, 70);
			inputAccount.fontSize = 22;
			inputAccount.color = "#ffffff";
			inputAccount.pos(sx + 25, sy + 90);
			inputAccount.name = "txt_phone";
			inputAccount.restrict = "0-9";
			registerForm.addChild(inputAccount);
			var inputVerifyCode = new TextInput;
			inputVerifyCode.prompt = uiLocallization.verifyCode;
			inputVerifyCode.size(265, 70);
			inputVerifyCode.fontSize = 22;
			inputVerifyCode.color = "#ffffff";
			inputVerifyCode.pos(sx + 25, sy + 200);
			inputVerifyCode.name = "txt_verifyCode";
			registerForm.addChild(inputVerifyCode);
			var getVerifyCodeBtn = new Button;
			getVerifyCodeBtn.setSkinImage("assets/main/login/getVerifyCode.png");
			getVerifyCodeBtn.stateNum = 2;
			getVerifyCodeBtn.pos(sx + 320, sy + 210);
			getVerifyCodeBtn.name = "getVerifyCode";
			getVerifyCodeBtn.setClickHandler(this, this.onClickGetVerifyCode);
			registerForm.addChild(getVerifyCodeBtn);
			var wordGetVerifyCode = uiUtils.createSprite("assets/main/login/word_getVerifyCode.png");
			wordGetVerifyCode.pos(6, 11);
			wordGetVerifyCode.name = "word";
			getVerifyCodeBtn.addChild(wordGetVerifyCode);
			var countGetVerifyCode = uiUtils.createSimpleText("", "count");
			countGetVerifyCode.pos(0, 10);
			countGetVerifyCode.fontSize = 25;
			countGetVerifyCode.width = 143;
			countGetVerifyCode.align = "center";
			getVerifyCodeBtn.addChild(countGetVerifyCode);
			var turnToLoginBtn = new Button;
			turnToLoginBtn.label = "已有账号";
			turnToLoginBtn.labelColors = "#ffffff,#ffffff,#ffffff";
			turnToLoginBtn.labelSize = 40;
			turnToLoginBtn.labelPadding = "-20,0,0,0";
			turnToLoginBtn.setSkinImage("assets/main/login/sss3-fs8.png");
			turnToLoginBtn.stateNum = 1;
			turnToLoginBtn.pivotToCenter();
			turnToLoginBtn.pos(sx + 36, 700);
			turnToLoginBtn.setClickHandler(this, this.onClickTurnToLogin);
			registerForm.addChild(turnToLoginBtn);
			var cRegisterBtn = new Button;
			cRegisterBtn.label = "继续注册";
			cRegisterBtn.labelColors = "#ffffff,#ffffff,#ffffff";
			cRegisterBtn.labelSize = 40;
			cRegisterBtn.labelPadding = "-20,0,0,0";
			cRegisterBtn.setSkinImage("assets/main/login/sss4-fs8.png");
			cRegisterBtn.stateNum = 1;
			cRegisterBtn.pivotToCenter();
			cRegisterBtn.pos(sx + 330, 700);
			cRegisterBtn.setClickHandler(this, this.onClickTurnToPasswordForm);
			registerForm.addChild(cRegisterBtn);
		...
		onClickUserLogin: function () {
			...
			this._locked = true;
			var loginForm = this.getChildByName("login_form");
			var txtUsername = loginForm.getChildByName("txt_username").text,
				txtPassword = loginForm.getChildByName("txt_password").text,
				cbUsername = loginForm.getChildByName("cb_username").selected,
				cbPassword = loginForm.getChildByName("cb_password").selected;
			if (txtUsername == loginForm.getChildByName("txt_username").prompt) txtUsername = "";
			if (txtPassword == loginForm.getChildByName("txt_password").prompt) txtPassword = "";
			if (txtUsername == "" || txtPassword == "") {
				topMessage.post(uiLocallization.account_pass_not_void);
				this._locked = false;
				return;
			}
			var account = txtUsername;
			if (isNaN(txtUsername)) account = txtUsername;
			else account = "00" + accountMgr.getUserData().country + "-" + txtUsername;
			this.runCommand("sms_login", account, txtPassword, cbUsername, cbPassword);
		},
