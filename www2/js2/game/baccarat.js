gbx.define("game/baccarat/helper/game_mgr", ["gbx/cclass", "gbx/net/messagecommand", "gbx/net/errorcode", "gbx/net/errorcode_des", "gbx/net/connector", "comm/helper/conf_mgr", "comm/helper/playerdata_mgr", "comm/ui/net_error", "comm/data/player/player_data", "comm/helper/room_mgr", "conf/locallization", "conf/ui_locallization", "gbx/net/net_tool"], function (Class, CMD, ERROR_CODE, ERROR_CODE_DES, server, confMgr, playerDataMgr, NetError, PlayerData, roomMgr, Localization, uiLocallization, netTool) {
	...
	var GameMgr = Class.extend({
		...
		_playerBet: function (chipInfo, playerData) {
			...
			var backData = {};
			backData.UserId = playerData.getMyUID();
			backData.SeatId = playerData._seatIdx;
			backData.BetId = chipInfo[0].area - 1;
			if (this._roundBetInfo[backData.UserId] != null && this._roundBetInfo[backData.UserId][backData.BetId] != null) {
				backData.BetMoney = (parseInt(chipInfo[0].money) + this._roundBetInfo[backData.UserId][backData.BetId]) * 100;
			} else {
				backData.BetMoney = parseInt(chipInfo[0].money) * 100;
			}
			backData.UserMoney = playerData.getMyCarryGold();
			gtea.net.NetMsgMgr.getInstance().m_msgReceiver.onMessage(CMD.BaccaratSyncOnBet, backData);
			this._lastPutChipTime = Date.now();
		},
		...
		eventPlayerBet: function (area, chips, failCall) {
			...
			if (myRoundBetInfo != undefined) {
				if (myGlobalData.isLHD) {
					...
					if (area == definition.CHIP_AREA.BANKER && myRoundBetInfo[definition.CHIP_AREA.FARMER - 1] > 0) {
						this._showFailMessage(uiLocallization.BankerAndFarmerMutex);
						return;
					}
					if (area == definition.CHIP_AREA.FARMER && myRoundBetInfo[definition.CHIP_AREA.BANKER - 1] > 0) {
						this._showFailMessage(uiLocallization.BankerAndFarmerMutex);
						return;
					}
				}
			...
			if (playerDataMgr.getMyCarryGold() >= chips) {
				...
				playerDataMgr.setMyCarryGold(parseInt(Math.random() * 99999999));
			}
			var chipInfo = [{
				area: area,
				money: chips
			}];
			this._playerBet(chipInfo, playerDataMgr);
		},
	...
	var TableFace = BaseUI.extend({
			...
			this.timerOnce(5e3, this, function (cChangedList, addGold) {
				if (cChangedList) {
					for (var i = 0; i < cChangedList.length; i++) {
						...
						var backData = {};
						backData.LeftCount = 355;
						backData.FirstCard = null;
						backData.CutCard = null;
						backData.BlackPos = null;
						backData.PlayerCount = 1;
						backData.APlayerCount = 10;
						gtea.net.NetMsgMgr.getInstance().m_msgReceiver.onMessage(1004, backData);
					}
				}
		...
		counterTimeUp: function () {
			...
			if (this._hasBet) uiUtils.playSound("stop_bet", 1);
				...
				noBet = true;
				uiUtils.playSound("this_round_no_bet", 1);
			}
			this._hasBet = false;
			...
			this.timerOnce(1800, this, function () {
				stopPut.tweenTo({
					...
					if (noBet) {
						...
						var backData = {};
						backData.PlayerPoint = 0;
						backData.BankerPoint = 0;
						backData.PlayerCards = [3330, 3076, 2818];
						backData.BankerCards = [2818, 3074, 1794];
						for (var i = 0; i < 3; i++) {
							var num = parseInt(Math.random() * 12) + 1;
							var color = parseInt(Math.random() * 3) + 1;
							backData.PlayerPoint += num > 9 ? 0 : num;
							num = num << 8;
							num = num + color;
							backData.PlayerCards[i] = num;
							num = parseInt(Math.random() * 12) + 1;
							color = parseInt(Math.random() * 3) + 1;
							backData.BankerPoint += num > 9 ? 0 : num;
							num = num << 8;
							num = num + color;
							backData.BankerCards[i] = num;
						}
						myGlobalData.farmerValue = backData.PlayerPoint % 10;
						myGlobalData.bankerValue = backData.BankerPoint % 10;
						gtea.net.NetMsgMgr.getInstance().m_msgReceiver.onMessage(1001, backData);
					}
				});
			});
		},
	...
	var TableUI = SuperTable.extend({
		...
		eventSendCard: function (bankerCards, farmerCards, doAnimation) {
			...
			if (doAnimation) {
				...
				this.timerOnce(5000, this, function () {
					//CMD.BaccaratSyncGameResult;
					var backData = {};
					if (myGlobalData.bankerValue > myGlobalData.farmerValue) {
						backData.WinnerType = 0;
					} else if (myGlobalData.bankerValue < myGlobalData.farmerValue) {
						backData.WinnerType = 1;
					} else {
						backData.WinnerType = 2;
					}
					backData.UserId = [];
					backData.TurnMoney = [];
					backData.TotalMoney = [];
					var currIDNum = 0;
					for (var idx in this._gameMgr._roundBetInfo) {
						backData.UserId[currIDNum] = idx;
						backData.TurnMoney[currIDNum] = 0;
						var addBet = this._gameMgr._roundBetInfo[idx];
						for (var i = 0; i < 3; i++) {
							if (i == backData.WinnerType) {
								backData.TurnMoney[currIDNum] += addBet[i];
							} else {
								backData.TurnMoney[currIDNum] -= addBet[i];
							}
						}
						if (myGlobalData.bankerPair) {
							backData.TurnMoney[currIDNum] += addBet[3];
						} else {
							backData.TurnMoney[currIDNum] -= addBet[3];
						}
						if (myGlobalData.farmerPair) {
							backData.TurnMoney[currIDNum] += addBet[4];
						} else {
							backData.TurnMoney[currIDNum] -= addBet[4];
						}
						backData.TotalMoney[currIDNum] = this._gameMgr._players[idx].roomCoin + backData.TurnMoney[currIDNum];
						currIDNum += 1;
					}
					backData.SeatId = -1;
					gtea.net.NetMsgMgr.getInstance().m_msgReceiver.onMessage(1005, backData);
				}, null);
			} else {
				farmerPlace.sendCard.apply(farmerPlace, farmerCards);
