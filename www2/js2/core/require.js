!
function(gbx) {
	var requirejs, require, define, queue;
	(function(global) {
		...
		req.createNode = function(config, moduleName, url, async) {
			var node = config.xhtml ? document.createElementNS("http://www.w3.org/1999/xhtml", "html:script") : document.createElement("script");
			node.type = config.scriptType || "text/javascript";
			node.charset = "utf-8";
			node.async = async == false ? false : true;
			return node;
		};
		req.load = function(context, moduleName, url) {
			var config = context && context.config || {},
				node;
			if (isBrowser) {
				node = req.createNode(config, moduleName, url, !/\.js$/.test(moduleName));
		...
		req.queue = function(deps, callback, modules) {
			if (deps.length) {
				require([deps.shift()], function(module) {
					if (modules == undefined) {
						modules = [];
					}
					modules.push(module);
					req.queue(deps, callback, modules);
				});
			} else {
				callback && isFunction(callback) && callback.apply(global, modules);
			}
		};
